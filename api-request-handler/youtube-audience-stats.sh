...
for ((i=0; i<${#id[@]}; i++)); do
  ...
  # curl command
  curl -H "Authorization: Bearer $token" "https://api.chartmetric.com/api/artist/${id[$i]}/youtube-audience-stats" \
  -o ../data/youtube-audience-stats/json/raw-json/${art[$i]}.json;
