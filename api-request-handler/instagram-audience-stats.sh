...
for ((i=0; i<${#id[@]}; i++)); do
  ...
  # curl command
  curl -H "Authorization: Bearer $token" https://api.chartmetric.com/api/artist/${id[$i]}/instagram-audience-stats \
  -o ../data/instagram-audience-stats/json/raw-json/${art[$i]}.json;
